...
<?php
	if(isset($_FILES['file'])){
		move_uploaded_file($_FILES['file']['tmp_name'],'files/'.$_SESSION['faculty'].'-students.csv');
	}
?>
<!DOCTYPE HTML>
<html>
		...
		$(document).ready(function(){
			...
			$('#grid').flexigrid({
				...
				buttons: [
					...
					{name: 'SAVE', bclass: 'add', onpress: function(){
						$.ajax({
							...
							data: { classcode: '<?php echo $_GET['class'] ?>' },
							success: function(i){
				...
				colModel : [
					...
					{display: 'COURSE', name : 'col', width : 70, align: 'center'},
					{display: 'YEAR LEVEL', name : 'col', width : 150, align: 'center'},
					{display: 'STATUS', name : 'col', width : 150, align: 'left'},
				],
				searchitems : [
				...
				title: 'UPLOAD STUDENTS : <span><?php echo $_GET['class'] ?></span>',
				height: windowHeight
			});
